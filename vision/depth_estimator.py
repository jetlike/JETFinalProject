...
def estimate_depth(frame):
    """Estimate a depth map from an RGB image using MiDaS"""
    ...
    with torch.no_grad():
        ...
        prediction = torch.nn.functional.interpolate(
            ...
            size=frame.shape[:2],  # Resize back to original frame size
            mode="bicubic",
            align_corners=False,
        ).squeeze()
...
def find_pointed_object_in_depth(depth_map, wrist, fingertip, max_steps=150, step_size=4, depth_jump_thresh=0.03):
    """
    Traces a pointing ray from fingertip and detects the first significant depth jump
    relative to the wrist. Returns the (x, y) pixel of the target.
    """
    h, w = depth_map.shape
    x0, y0 = wrist
    x1, y1 = fingertip
